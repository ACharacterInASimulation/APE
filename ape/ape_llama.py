...
def llama_attention_prefill_prefix(
    self,
...
def llama_attention_prefill_query(
    self,
    hidden_states: torch.Tensor,
    attention_mask: Optional[torch.Tensor] = None,
    position_ids: Optional[torch.LongTensor] = None,
...
def enable_llama_attention_prefill_prefix(model):
    for name, module in reversed(model._modules.items()):
        if len(list(module.children())) > 0:
            enable_llama_attention_prefill_prefix(
                module,
            )

        if isinstance(module, LlamaAttention):
            model._modules[name].forward = types.MethodType(
                llama_attention_prefill_prefix, model._modules[name]
            )
...
def enable_llama_attention_prefill_context(model):
    for name, module in reversed(model._modules.items()):
        if len(list(module.children())) > 0:
            enable_llama_attention_prefill_context(
                module,
            )
